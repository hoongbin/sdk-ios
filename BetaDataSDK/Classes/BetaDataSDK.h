...
@class BetaDataAnalyticsPeople;

/**
 * @abstract
 * 在 DEBUG 模式下，发送错误时会抛出该异常
 */
@interface BetaDataDebugException : NSException

@end

@protocol BTUIViewAutoTrackDelegate

//UITableView
@optional
-(NSDictionary *) betadataAnalytics_tableView:(UITableView *)tableView autoTrackPropertiesAtIndexPath:(NSIndexPath *)indexPath;

//UICollectionView
@optional
-(NSDictionary *) betadataAnalytics_collectionView:(UICollectionView *)collectionView autoTrackPropertiesAtIndexPath:(NSIndexPath *)indexPath;

//@optional
//-(NSDictionary *) betadataAnalytics_alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex;
//
//@optional
//-(NSDictionary *) betadataAnalytics_actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex;
@end
 ...
 *
 * Debug模式有三种选项:
 *   BetaDataDebugOff - 关闭 DEBUG 模式
 *   BetaDataDebugOnly - 打开 DEBUG 模式，但该模式下发送的数据仅用于调试，不进行数据导入
 *   BetaDataDebugAndTrack - 打开 DEBUG 模式，并将数据导入到 SensorsAnalytics 中
 */
typedef NS_ENUM(NSInteger, BetaDataDebugMode) {
    BetaDataDebugOff,
    BetaDataDebugOnly,
    BetaDataDebugAndTrack,
};
 ...
 * @discuss
 * 时间单位有以下选项：
 *   BetaDataTimeUnitMilliseconds - 毫秒
 *   BetaDataTimeUnitSeconds - 秒
 *   BetaDataTimeUnitMinutes - 分钟
 *   BetaDataTimeUnitHours - 小时
 */
typedef NS_ENUM(NSInteger, BetaDataTimeUnit) {
    BetaDataTimeUnitMilliseconds,
    BetaDataTimeUnitSeconds,
    BetaDataTimeUnitMinutes,
    BetaDataTimeUnitHours
};
 ...
 *
 * @discussion
 *   BetaDataEventTypeAppStart - $AppStart
 *   BetaDataEventTypeAppEnd - $AppEnd
 *   BetaDataEventTypeAppClick - $AppClick
 *   BetaDataEventTypeAppViewScreen - $AppViewScreen
 */
typedef NS_OPTIONS(NSInteger, BetaDataAnalyticsAutoTrackEventType) {
    BetaDataEventTypeNone      = 0,
    BetaDataEventTypeAppStart      = 1 << 0,
    BetaDataEventTypeAppEnd        = 1 << 1,
    BetaDataEventTypeAppClick      = 1 << 2,
    BetaDataEventTypeAppViewScreen = 1 << 3,
};
 ...
 *
 * @discussion
 *   BetaDataNetworkTypeNONE - NULL
 *   BetaDataNetworkType2G - 2G
 *   BetaDataNetworkType3G - 3G
 *   BetaDataNetworkType4G - 4G
 *   BetaDataNetworkTypeWIFI - WIFI
 *   BetaDataNetworkTypeALL - ALL
 */
typedef NS_OPTIONS(NSInteger, BetaDataNetworkType) {
    BetaDataNetworkTypeNONE      = 0,
    BetaDataNetworkType2G       = 1 << 0,
    BetaDataNetworkType3G       = 1 << 1,
    BetaDataNetworkType4G       = 1 << 2,
    BetaDataNetworkTypeWIFI     = 1 << 3,
    BetaDataNetworkTypeALL      = 0xFF,
};
 ...
 *  BetaDataSDK 初始化配置
 */
@interface BTConfigOptions : NSObject
...
- (instancetype)initWithServerURL:(nonnull NSString *)serverURL
                    ...
                    launchOptions:(nullable NSDictionary<NSString *, id> *)launchOptions NS_DESIGNATED_INITIALIZER;
 ...
 *
 * @discussion
 * 使用 BetaDataSDK 类来跟踪用户行为，并且把数据发给所指定的 SensorsAnalytics 的服务。
 * 它也提供了一个 BetaDataAnalyticsPeople 类型的 property，用来访问用户 Profile 相关的 API。
 */
@interface BetaDataSDK : NSObject

/**
 * @property
 *
 * @abstract
 * 对 BetaDataAnalyticsPeople 这个 API 的访问接口
 */
@property (atomic, readonly, strong) BetaDataAnalyticsPeople *people;
...
/**
 * @abstract
 * 根据传入的配置，初始化并返回一个 BetaDataSDK 的单例
 *
 ...
 *
 * @param webView 当前 WebView，支持 WKWebView;  UIWebView已废弃
 *
 ...
 *
 * @param networkType BetaDataNetworkType
 */
...
/**
 * @abstract
 * 判断某个 ViewController 是否被默认拉黑
 *
 * @param controller  UIViewController
 * |
 * @return YES:可以报送;  NO:是拉黑的VC
*/
 ...
 */
- (void)trackInstallationWithProperties:(nullable NSDictionary *)propertyDict;
 ...
 */
- (void)trackInstallationWithProperties:(nullable NSDictionary *)propertyDict disableCallback:(BOOL)disableCallback;
 ...
 */
- (void)setDebugMode:(BetaDataDebugMode)debugMode __attribute__((deprecated("已过时，建议动态开启调试模式")));
 ...
 *
 * @discussion
 * 根据需要决定是否开启 SDK log , BetaDataDebugOff 模式默认关闭 log
 * BetaDataDebugOnly  BetaDataDebugAndTrack 模式默认开启log
 *
...
/**
 * @class
 * BetaDataAnalyticsPeople 类
 *
 * @abstract
 * 用于记录用户 Profile 的 API
 *
 * @discussion
 * <b>请不要自己来初始化这个类.</b> 请通过 BetaDataSDK 提供的 people 这个 property 来调用
 */
@interface BetaDataAnalyticsPeople : NSObject
