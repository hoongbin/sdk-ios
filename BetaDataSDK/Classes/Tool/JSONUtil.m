...
- (NSData *)JSONSerializeObject:(id)obj {
    ...
    @catch (NSException *exception) {
        BTLog(@"%@ exception encoding api data: %@", self, exception);
    }
    if (error) {
        BTLog(@"%@ error encoding api data: %@", self, error);
    }
    return data;
}
...
- (id)JSONSerializableObjectForObject:(id)obj {
    ...
    if ([newObj isKindOfClass:[NSDictionary class]]) {
        ...
        for (id key in newObj) {
            ...
            if (![key isKindOfClass:[NSString class]]) {
                ...
                BTLog(@"%@ warning: property keys should be strings. got: %@. coercing to: %@", self, [key class], stringKey);
            } else {
    ...
    BTLog(@"%@ warning: property values should be valid json types. got: %@. coercing to: %@", self, [newObj class], s);
    return s;
}
