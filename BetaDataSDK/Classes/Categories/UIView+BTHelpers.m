...
@implementation UIView (BTHelpers)
...
- (int)jjf_fingerprintVersion {
    return [self bt_fingerprintVersion];
}
...
- (UIImage *)bt_snapshotForBlur {
    UIImage *image = [self bt_snapshotImage];
...
- (void)setBetaDataViewID:(id)object {
    objc_setAssociatedObject(self, @selector(betaDataViewID), [object copy], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
