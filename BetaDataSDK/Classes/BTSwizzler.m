...
static void sa_swizzledMethod_2(id self, SEL _cmd) {
    ...
    BTSwizzle *swizzle = (BTSwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];
    if (swizzle) {
...
static void sa_swizzledMethod_4(id self, SEL _cmd, id arg, id arg2) {
    Method aMethod = class_getInstanceMethod([self class], _cmd);
    BTSwizzle *swizzle = (BTSwizzle *)[swizzles objectForKey:(__bridge id)((void *)aMethod)];
    if (swizzle) {
...
+ (void)printSwizzles {
    ...
    BTSwizzle *swizzle;
    while((swizzle = (BTSwizzle *)[en nextObject])) {
        BTLog(@"%@", swizzle);
    }
}

+ (BTSwizzle *)swizzleForMethod:(Method)aMethod {
    return (BTSwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];
}
...
+ (void)setSwizzle:(BTSwizzle *)swizzle forMethod:(Method)aMethod {
...
//        [NSException raise:@"SwizzleException" format:@"Cannot find method for %@ on %@", NSStringFromSelector(aSelector), NSStringFromClass(aClass)];
        BTLog(@"SwizzleException:Cannot find method for %@ on %@",NSStringFromSelector(aSelector), NSStringFromClass(aClass));
        return;
    }
    ...
    [BTSwizzler swizzleSelector:aSelector onClass:aClass withBlock:aBlock andSwizzleMethod:swizzledMethod named:aName];
}
...
+ (void)swizzleSelector:(SEL)aSelector
    ...
    BTSwizzle *swizzle = [self swizzleForMethod:aMethod];
    ...
    if (isLocal) {
        if (!swizzle) {
            ...
            @try {
                swizzle = [[BTSwizzle alloc] initWithBlock:aBlock named:aName forClass:aClass selector:aSelector originalMethod:originalMethod];
        ...
        BTSwizzle *newSwizzle = [[BTSwizzle alloc] initWithBlock:aBlock named:aName forClass:aClass selector:aSelector originalMethod:originalMethod];
