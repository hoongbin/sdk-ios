...
- (id)initWithFilePath:(NSString *)filePath {
    ...
    if (sqlite3_initialize() != SQLITE_OK) {
        BTLog(@"failed to initialize SQLite.");
        return nil;
    }
    if (sqlite3_open_v2([filePath UTF8String], &_database, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL) == SQLITE_OK ) {
        ...
        if (sqlite3_exec(_database, [_sql UTF8String], NULL, NULL, &errorMsg)==SQLITE_OK) {
            BTLog(@"Create dataCache Success.");
        } else {
            BTLog(@"Create dataCache Failure %s",errorMsg);
            return nil;
        }
        ...
        BTLog(@"SQLites is opened. current count is %ul", _messageCount);
    } else {
        if (_dbStmtCache) CFRelease(_dbStmtCache);
        _dbStmtCache = NULL;
        
        BTLog(@"failed to open SQLite db.");
        return nil;
    }
    return self;
}
...
- (void)addObejct:(id)obj withType:(NSString *)type {
    UInt64 maxCacheSize = [[BetaDataSDK sharedInstance] getMaxCacheSize];
    if (_messageCount >= maxCacheSize) {
        BTLog(@"touch MAX_MESSAGE_SIZE:%d, try to delete some old events", maxCacheSize);
        BOOL ret = [self removeFirstRecords:100 withType:@"Post"];
        if (ret) {
            _messageCount = [self sqliteCount];
        } else {
            BTLog(@"touch MAX_MESSAGE_SIZE:%d, try to delete some old events FAILED", maxCacheSize);
            return;
        }
    }
    ...
    if (insertStatement) {
        ...
        @try {
            ...
            BTLog(@"Found NON UTF8 String, ignore");
            return;
        }
        rc = sqlite3_step(insertStatement);
        if(rc != SQLITE_DONE) {
            BTLog(@"insert into dataCache fail, rc is %d", rc);
        } else {
            _messageCount ++;
            BTLog(@"insert into dataCache success, current count is %lu", _messageCount);
        }
    } else {
        BTLog(@"insert into dataCache error");
    }
}
    ...
    if(stmt) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            @try {
                ...
                if (!jsonChar) {
                    BTLog(@"Failed to query column_text, error:%s", sqlite3_errmsg(_database));
                    return nil;
                }
                ...
                if (!err && eventDict) {
                    ...
                    //[eventDict setValue:@(time) forKey:BT_EVENT_FLUSH_TIME];
        ...
        BTLog(@"Failed to prepare statement, error:%s", sqlite3_errmsg(_database));
        return nil;
    }
...
- (void) deleteAll {
    ...
    @try {
        if (sqlite3_exec(_database, [query UTF8String], NULL, NULL, &errMsg) != SQLITE_OK) {
            BTLog(@"Failed to delete record msg=%s", errMsg);
        }
    } @catch (NSException *exception) {
        BTLog(@"Failed to delete record exception=%@",exception);
    }
...
- (NSInteger) sqliteCount {
    ...
    if(statement) {
        ...
        BTLog(@"Failed to prepare statement");
    }
    return count;
}
...
- (sqlite3_stmt *) dbCacheStmt:(NSString *)sql {
    ...
    if (!stmt) {
        ...
        if (result != SQLITE_OK) {
            BTLog(@"%s line:%d sqlite stmt prepare error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_database));
            return NULL;
        }
